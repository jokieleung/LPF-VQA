...
class BaselineNet(nn.Module):
    ...
    def forward(self, batch):
        ...
        #     classify_linear = self.classif_module.linears[-1]
        #     weight_norm_per_class = torch.norm(classify_linear.weight,dim=1) 
        #     for i in range(torch.tensor(weight_norm_per_class.shape).item()):
        #         print("class id:%d, weight_norm: %.5f" % (i, weight_norm_per_class[i]))
